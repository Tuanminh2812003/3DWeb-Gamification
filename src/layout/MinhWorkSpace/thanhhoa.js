...
const PuzzleGame = lazy(() => import('../GamePuzzle/thanhhoa1.js'));
const PuzzleGame2 = lazy(() => import('../GamePuzzle/thanhhoa2.js'));
...
const modelsConfig = useMemo(
    () => [
        ...
        {
            path: "/Farm/bonus img thanhhoa.glb",
            position: [0, 0, 0],
            rotation: [0, 0, 0],
            scale: [1, 1, 1],
            clickable: false,
    ...
    // mảng items các bức tranh để làm tour
        ...
        const [nextPath, setNextPath] = useState("/hue"); // Path chuyển tiếp
    ...
    const backgroundAudioRef = useRef(null);
        ...
        useEffect(() => {
            const audio = new Audio('/Farm/Music/thanhhoa.MP3');
            audio.loop = true;
            audio.volume = 1;
        ...
        <>
            <CameraProvider>
                <div className='main'>
                    ...
                    {showQuestionPopup && (
                            <QuestionPopup
                            question="Khu di tích lịch sử Lam Kinh thuộc huyện nào của tỉnh Thanh Hóa?"
                            answers={["Đông Sơn", "Vĩnh Lộc", "Thọ Xuân", "Triệu Sơn"]}
                            correctAnswer="Thọ Xuân"
                            onClose={handleCloseQuestionPopup}
                            />
                        )}
                        {showQuestionPopup2 && (
                            <QuestionPopup
                            question="Cố đô Lam Kinh - một di tích lịch sử nổi tiếng ở Thanh Hóa gắn liền với vị vua nào ?"
                            answers={["Lê Thái Tổ", "Lê Thánh Tông", "Lý Thái Tổ", "Hồ Quý Ly"]}
                            correctAnswer="Lê Thái Tổ"
                            onClose={handleCloseQuestionPopup2}
                            />
                        )}
                        {showQuestionPopup3 && (
                            <QuestionPopup
                            question="Di tích Thành Nhà Hồ được UNESCO công nhận là Di sản Văn Hóa Thế giới vào năm nào ?"
                        ...
                        {showQuestionPopup4 && (
                            <QuestionPopup
                            question="Loại hình kiến trúc đặc biệt nào nổi bật trong quần thể di tích Thành Nhà Hồ ?"
                            answers={["Thành lũy bằng đất nung", "Thành đá lớn khép kín không cần vữa", "Thành tre bao quanh đồi núi", "Thành gạch xây theo lối Đông Dương"]}
                            correctAnswer="Thành đá lớn khép kín không cần vữa"
                ...
                <div className="popup-overlay">
                    <div className="popup-content">
                        <h2>Chúc mừng bạn!</h2>
                        <p>Bạn đã qua màn 4, chuyển đến màn tiếp theo!</p>
            ...
            {/* Respondsive */}
            <ResizeHandler updateItemsForScreenSize={updateItemsForScreenSize} level="ThanhHoa" />
            {/* Respondsive */}
        </>
    )
}

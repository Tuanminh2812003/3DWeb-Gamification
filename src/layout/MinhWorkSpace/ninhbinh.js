...
const PuzzleGame = lazy(() => import('../GamePuzzle/ninhbinh1.js'));
const PuzzleGame2 = lazy(() => import('../GamePuzzle/ninhbinh2.js'));
...
const modelsConfig = useMemo(
    () => [
        ...
        {
            path: "/Farm/bonus img ninhbinh.glb",
            position: [0, 0, 0],
            rotation: [0, 0, 0],
            scale: [1, 1, 1],
            clickable: false,
    ...
    // mảng items các bức tranh để làm tour
        ...
        const [nextPath, setNextPath] = useState("/thanhhoa"); // Path chuyển tiếp
    ...
    const backgroundAudioRef = useRef(null);
        ...
        useEffect(() => {
            const audio = new Audio('/Farm/Music/ninhbinh.MP3');
            audio.loop = true;
            audio.volume = 1;
        ...
        <>
            <CameraProvider>
                <div className='main'>
                    ...
                    {showQuestionPopup && (
                            <QuestionPopup
                            question="Dòng sông nào đóng vai trò quan trọng trong việc tạo nên vẻ đẹp non nước hữu tình của khu du lịch Tam Cốc - Bích Động?"
                            answers={["Sông Hồng", "Sông Mã", "Sông Hoàng Long", "Sông Ngô Đồng"]}
                            correctAnswer="Sông Ngô Đồng"
                            onClose={handleCloseQuestionPopup}
                            />
                        )}
                        {showQuestionPopup2 && (
                            <QuestionPopup
                            question="Công trình kiến trúc tôn giáo độc đáo nào ở Ninh Bình được xây dựng hoàn toàn bằng đá tự nhiên, với những đường nét chạm khắc tinh xảo mang đậm phong cách kiến trúc Gothic phương Tây?"
                            answers={["Chùa Bái Đính", "Nhà thờ đá Phát Diệm", "Đền Thái Vi", "Chùa Non Nước"]}
                            correctAnswer="Nhà thờ đá Phát Diệm"
                            onClose={handleCloseQuestionPopup2}
                            />
                        )}
                        {showQuestionPopup3 && (
                            <QuestionPopup
                            question="Di tích lịch sử nào ở Ninh Bình từng là kinh đô của Việt Nam dưới thời các triều đại Đinh, Tiền Lê và Lý, với những dấu tích còn lại của cung điện và đền thờ các vị vua?"
                            answers={["Tam Cốc", "Bích Động", "Cố đô Hoa Lư", "Tràng An"]}
                            correctAnswer="Cố đô Hoa Lư"
                            onClose={handleCloseQuestionPopup3}
                            />
                        )}
                        {showQuestionPopup4 && (
                            <QuestionPopup
                            question="Ngọn núi nào nổi tiếng với chùa Bái Đính, một trong những ngôi chùa lớn nhất Đông Nam Á? "
                            answers={["Núi Non Nước ", "Núi Kỳ Lân ", "Núi Dục Thúy ", "Núi Bái Đính"]}
                            correctAnswer="Núi Bái Đính"
                ...
                <div className="popup-overlay">
                    <div className="popup-content">
                        <h2>Chúc mừng bạn!</h2>
                        <p>Bạn đã qua màn 3, chuyển đến màn tiếp theo!</p>
            ...
            {/* Respondsive */}
            <ResizeHandler updateItemsForScreenSize={updateItemsForScreenSize} level="NinhBinh" />
            {/* Respondsive */}
        </>
    )
}
